...
const PRODUCTS = {
  wstart: {
    ...
    colors: [
      { name: 'Rose', hex: '#d650a5', img: 'ROSE_FRONT_NICE' },
      { name: 'Hot Pink', hex: '#dea3d6', img: 'HOT_PINK_FRONT_NICE' },
      { name: 'Lavendel', hex: '#B8A6D9', img: 'LAVENDEL_FRONT_NICE' },
      { name: 'Lila', hex: '#8662A8', img: 'LILA_FRONT_NICE' }
    ],
...
function renderPDP() {
  ...
  const colorHtml = p.hasColor && p.colors.length ? `<div style="margin-bottom:24px"><div class="opt-label">Välj färg</div><div class="color-grid" id="pdp-colors">${p.colors.map((c) => `<div class="cswatch ${c.name === pdpColor ? 'sel' : ''}" style="background:${c.hex}" data-name="${c.name}" onclick="pickColor(this)" title="${c.name}"></div>`).join('')}</div><div class="color-chosen" id="pdp-color-name">${pdpColor || p.colors[0].name}</div></div>` : ''; const specRows = p.specs.map(([k, v]) => `<div class="spec-row"><span class="spec-key">${k}</span><span class="spec-val">${v}</span></div>`).join('');
